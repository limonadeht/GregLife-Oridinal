...
public class ItemBoiler extends ItemBlock implements IFluidContainerItem{

	public TileBoiler tileB;

	public ItemBoiler(Block block){
		super(block);
	}

	@SuppressWarnings({ "rawtypes", "unchecked" })
	public void addInformation(ItemStack itemStack, EntityPlayer player, List list, boolean bool){
		BlockBoiler boiler = (BlockBoiler)this.field_150939_a;

		FluidTank fakeTank = readTank(itemStack);
		FluidStack fluidStack = fakeTank.getFluid();

		list.add("I am Boiler!");
		list.add("Max Storage: " + boiler.maxTankStorage + " mb");
		if(fluidStack != null && fluidStack.amount > 0){
			...
			list.add(EnumChatFormatting.GREEN + fluidStack.getLocalizedName() + " " + String.format("%d mB (%.0f%%)", fluidStack.amount, percent));*/
			list.add(tileB.waterTank.getFluid().getLocalizedName() + tileB.waterTank.getFluidAmount() + " / 16000");
			list.add(tileB.steamTank.getFluid().getLocalizedName() + tileB.steamTank.getFluidAmount() + " / 16000");
		}else{
	...
	public static ItemStack createFilledTank(Fluid fluid) {
		final int tankCapacity = TileBoiler.getTankCapacity();
		...
		ItemStack item = new ItemStack(Contents.boiler);
		saveTank(item, tank);
		return item;
	}

	private static FluidTank readTank(ItemStack stack) {
		FluidTank tank = new FluidTank(TileBoiler.getTankCapacity());

		final NBTTagCompound itemTag = stack.getTagCompound();
		if (itemTag != null && itemTag.hasKey("waterTank")) {
			tank.readFromNBT(itemTag.getCompoundTag("waterTank"));
			return tank;
		}
		if (itemTag != null && itemTag.hasKey("steamTank")) {
			tank.readFromNBT(itemTag.getCompoundTag("steamTank"));
			return tank;
		}
	...
	private static void saveTank(ItemStack container, FluidTank tank) {
		if (tank.getFluidAmount() > 0) {
			...
			itemTag.setTag("steamTank", tankTag);
		} else {
