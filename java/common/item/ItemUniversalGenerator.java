...
public class ItemUniversalGenerator extends ItemBlock implements IFluidContainerItem{

	public ItemUniversalGenerator(Block block){
		super(block);
	}

	@SuppressWarnings({ "rawtypes", "unchecked" })
	public void addInformation(ItemStack itemStack, EntityPlayer player, List list, boolean bool){
		BlockUniversalGenerator generator = (BlockUniversalGenerator)this.field_150939_a;

		FluidTank fakeTank = readTank(itemStack);
		FluidStack fluidStack = fakeTank.getFluid();

		list.add("Universal Power!");
		...
		if(SimplyGenerators.commonProxy.isShiftKeyDown()){
			list.add("If this Generator is fill from 'Furnace Fuel' our 'Liquid'");
	...
	public static ItemStack createFilledTank(Fluid fluid) {
		...
		ItemStack item = new ItemStack(Contents.universalGenerator);
		saveTank(item, tank);
		return item;
	}
